...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#btnBomComponent").bind('click', function () {
			location.assign("bomComponentList?platformVersion="+$("select option:selected").val());
		});

		$("#btnExcel").bind('click', function () {
			location.assign("../excel/excelDown?target=bomLicenseList&platformVersion="+$("select option:selected").val());
		});		
		...
		$(':radio[name="rdoStatus"]').bind('click', function () {
			...
			location.assign("bomLicenseList?platformVersion="+$("select option:selected").val()+"&status="+status);
		});
	...
	{
		var fossWin = window.open("../popup/fossPop?platformVersion="+$("select option:selected").val(), 'popName', 'scrollbars=yes, width=640, height=600, resizable=no');
	...
	function openLicenseProjectPop(license)
	{
		var platformVersoion = $("select option:selected").val();
		var url = "../popup/licenseProjectPop?platformVersion="+platformVersoion+"&license="+encodeURIComponent(license);
...
<body>
	...
	<div class="detail">
		<div class="container">
			<h3>
				Detail (Total ${fn:length(bomList)} License)
			</h3>
			<table class="search">
				<tbody>
					<tr>
						<th align="left" width="70%">
							<button id="btnBomComponent" type="button" class="btn btn-sm btn-default">Component 기준</button>&nbsp;&nbsp;
							<button id="btnBomLicense" type="button" class="btn btn-sm btn-primary">License 기준</button>
						</th>
						<th>
						</th>							
			...
			<table id="listTable" class="sortable table table-hover table-striped table-bordered">
				<thead>
					<tr style="cursor: pointer;">
						<th data-sort="string" align="center">License</th>
				...
				<tbody>
					<c:forEach items="${bomList}" var="bom" varStatus="status">
						<tr <c:if test="${status.count%2 != 0}"> class="even"</c:if><c:if test="${status.count%2 == 0}">class="odd"</c:if>>
							...
							<td align="right" style="cursor: pointer;" onclick="openLicenseProjectPop('${bom.license}')"><span style="color: blue;">${bom.relatedProject}</span></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
